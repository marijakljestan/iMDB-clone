...
export class MoviePageComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.movieService.getMovieById(this.movieId).subscribe(response => {
        ...
        this.movieService.getMoviesWithSameGenre(this.movieId).subscribe(data => {
            this.moviesRecommended = [...data];
            for(let movie of this.moviesRecommended) {
                    movie.notInWatchlist = true;
            }
            this.moviesRecommended.sort(() => Math.random() - 0.5);
        });
    });
  }
}
